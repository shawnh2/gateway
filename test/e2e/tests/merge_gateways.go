...
var MergeGatewaysTest = suite.ConformanceTest{
	...
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		...
		t.Run("conflicted gateways should have the same address", func(t *testing.T) {
			gw3HostPort, err := kubernetes.WaitForGatewayAddress(t, suite.Client, suite.TimeoutConfig, gw3NN)
			if err != nil {
				t.Errorf("failed to get the address of gateway %s", gw3NN.String())
			}
			...
			if gw3Addr != gw4Addr {
				t.Errorf("gateway %s has inconsistent address %s with other gateways %s with address %s",
					gw3NN.String(), gw3Addr, gw4NN.String(), gw4Addr)
				t.FailNow()
			}
		})
		...
		t.Run("merged gateways should receive traffic from different http routes", func(t *testing.T) {
			if len(consistentGatewayAddress) == 0 {
				t.Error("failed to get consistent gateway address")
				t.FailNow()
			}
